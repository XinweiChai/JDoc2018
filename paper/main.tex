...
\ed{COMUE L'UNAM\\\'Ecole Doctorale STIM\\Sciences et Technologies de\\l'Information et 
Math\'ematiques} 
\spec{Informatique}
...
Given global initial state $s\in \mathbf{L}$ and global final state $\Omega\in \mathbf{L}$, a trajectory $t$ from $s$ to $\Omega$ is a sequence of transitions in $\mathbf{T}$ that can be fired successively. Analogously, the trajectory $t$ from $s$ to a local final state $\omega\in L_a$ is also defined.%The set of all trajectories from $s$ is denoted $\mathbf{Tr} (s)$.
\end{definition}
